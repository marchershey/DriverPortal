...
<div class="flex h-full" x-data="{ mobilemenu: false }">
                            ...
                            <button @click="open = !open" type="button" class="group w-full rounded-md px-3.5 py-2 text-sm leading-5 font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-500 focus:outline-none focus:bg-gray-200 focus:border-blue-300 active:bg-gray-50 active:text-gray-800 transition ease-in-out duration-150" id="options-menu" aria-haspopup="true" x-bind:aria-expanded="open">
                                <div class="flex w-full justify-between items-center">
                                    <div class="flex min-w-0 items-center justify-between space-x-3">
                                        <img class="w-10 h-10 rounded-full flex-shrink-0" src="{{ Auth::user()->avatar() }}" alt="">
                                        <div class="flex-1 min-w-0 text-left">
                                            <h2 class="text-white group-hover:text-gray-900 group-focus:text-gray-900 text-sm leading-5 font-medium truncate">{{ Auth::user()->fullName() }}</h2>
                                            <p class="text-gray-500 text-xs leading-5 truncate">{{ Auth::user()->email }}</p>
                                        </div>
                                    </div>
    ...
    {{-- app --}}
    <div class="flex flex-col w-full md:overflow-y-auto h-screen">
        ...
        {{-- content --}}
        <div id="content" class="overflow-y-auto md:overflow-y-visible">
            <div class="max-w-screen-md p-4 sm:p-10 pb-10">
